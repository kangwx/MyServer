...
import cn.kang.web.formbean.CuisineForm;

@Controller("/cuisine/show")
public class CuisineAction extends DispatchAction {
	@Resource
	...
	public ActionForward showList(ActionMapping mapping, ActionForm form,
			...
			throws Exception {
		System.out.println("show cuisine list");
		response.setContentType("text/plain");
		response.setCharacterEncoding("UTF-8");
		PrintWriter out = response.getWriter();
		CuisineForm cuisineForm = (CuisineForm) form;
		int page = cuisineForm.getPage();
		System.out.println("page:" + page);
		QueryResult<Cuisine> query = cuisineService.getScrollDataByPage(page);
		JSONArray array = null;
		if (query != null) {
			List<Cuisine> list = query.getResultlist();
